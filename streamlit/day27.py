...
from streamlit_elements import elements, dashboard, mui, editor, medai, lazy, sync, nivo
...
with elements("demo"):
    ...
    with dashboard.Grid(layout, draggableHandle=".draggable"):
        ...
        # We want to make card's content take all the height availbale by setting flex CSS value to 1.
        # We also wnat card's contet to shirk when the card is shrinked by setting minHeight to 0.
    ...
    with mui.Card(key="chart", sx={"display": "flex", "flexDirection": "column"}):  
        ...
        with mui.CardContent(sx={"flex": 1, "minHeught":0}):
        ...
        # Thrd eleement of the dashboard, the Medai player.

        with mui.Card(key="medai", sx={"display":"flex","flexDirection": ""}):
            mui.CardHeader(title="Medai Player", className="draggable")
            with mui.CardContent(sx={"flex": 1, "minHeight": 0}):
