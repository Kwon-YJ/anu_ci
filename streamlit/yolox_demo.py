...
def imageflow_demo(predictor, vis_folder, current_time, args):
    ...
    while True:
        ...
        if ret_val:
            ...
            image_list.append(result_frame)

            #print(len(image_list))

            # save_dir = os.path.join(save_folder, f"{str(time.time())}.jpg")
            cv2.imwrite(f"./streamlit_save_dir/{str(time.time())}.jpg", result_frame)
    ...
    if st.sidebar.button("camera on"):
        while 1:
            ...
            if len(image_list) != 0:
                ...
                placeholder.image(img, width=600)
                os.remove(f'streamlit_save_dir//{img}')
...
if __name__ == "__main__":
    ...
    args = make_parser_custom().parse_args()
    exp = get_exp(args.exp_file, args.name)
    
    
    asyncio.run(main(exp, args))
    asyncio.run(web_bowser())
