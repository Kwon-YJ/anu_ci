...
def vertical_overlay(targets, BG_img, BG_tag, vertical_value):
    ...
    for idx, img in enumerate(targets):
        ...
        cntrX = vertical_value / 3840
        cntrY = (vpos + (0.5*rows)) / 2160
        width = cols / 3840
        height = rows / 2160
        txt_data[0] = txt_data[0] + f'{class_id} {cntrX} {cntrY} {width} {height}\n'
...
def horizontal_overlay(targets, BG_img, BG_tag, horizontal_value):
    ...
    for idx, img in enumerate(targets):
        ...
        if hpos + rows > 3840:
            break
        if hpos+rows>1280-260 and hpos+rows<1280+260 or hpos+rows>2560-260 and hpos+rows<2560+260:
            hpos += cols + 5
            continue
        ...
        classes = ['apis', 'black', 'crabro', 'simil', 'jangsu', 'ggoma']
        class_id = classes.index(text)
        cntrX = (hpos + (0.5*cols)) / 3840
        cntrY = horizontal_value / 2160
        width = cols / 3840
        height = rows / 2160
        txt_data[0] = txt_data[0] + f'{class_id} {cntrX} {cntrY} {width} {height}\n'
...
for i in range(150):
    ...
    f = open(f'./tile_test_/{file_name}.txt', 'w')
    f.write(txt_data[0])
    f.close()
    txt_data = ''
