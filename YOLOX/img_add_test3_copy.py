...
def img_add(targets):
    ...
    for i in range(32):
        for j in range(18):
            ...
            if ratio[0] > ratio[1]:
                width = ratio[1] * 120 / 3840
                height = 120 / 2160
            else:
                width = 120 / 3840
                height = ratio[0] * 120 / 2160
    ...
    if platform.system() == 'Linux':
        cv2.imwrite(f'{os.getcwd()}/tile_test_4/{file_name}.jpg', BG_img)
    elif platform.system() == 'Windows':
        cv2.imwrite(f'{os.getcwd()}\\tile_test_4\\{file_name}.jpg', BG_img)

    f = open(f'./tile_test_4/{file_name}.txt', 'w')
    f.write(txt_data)
    f.close()
